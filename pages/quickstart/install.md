...
| その他のファイル | 読み取り | 404(r-----r--) |

その他のファイルの権限を `404(r-----r--)` とした場合にはFTPユーザに対しての書き込み権限がない状態となります。
FTPユーザで書き込みが必要な場合には `604(rw----r--)` のパーミッションをご検討ください。
...
EC-CUBEのインストール時に設定が記載された `.env` ファイルが作成されます。
EC-CUBEの重要な設定が記載されていますので、書き込み権限を制限したほうが安全です。
本番環境の `.env` ファイルの利用については[本番環境での .env ファイルの利用について](#link8)も合わせてご覧ください。
