...
1. 成功メッセージの表示方法を説明します。
      ...
      - 前述の通り、主に管理画面で利用しますが、処理が成功した際に表示したいメッセージを第一引数に渡します。
...
1. 取得結果がなくても、**array()**が必ず格納されているため、結果判定を行わず、**render**メソッドの第二引数に、取得結果変数を渡しています。
...
1. オブジェクトを全て走査するために「for ～ in」構文を用います。
